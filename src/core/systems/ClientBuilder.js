...
export class ClientBuilder extends System {
  ...
  async addIFC(file, transform) {
    ...
    // Generate IFC control script (ES5-compatible, similar to FramedImage example)
    // Note: This script is also available as examples/ifc-filter.hyp.js for reference
    const ifcScriptCode = [
      ...
      '    onClick: function() {',
      '      var root = app.root || app.get(\'$root\')',
      '      if (root && root.traverse) {',
      '        root.traverse(function(node) {',
      '          if (node.userData && node.userData.ifcType) {',
      ...
      'var checkCount = 0',
      'var maxChecks = 1200', // Increase to 20 minutes (60 FPS * 60 seconds * 20)
      'if (!setupIFCFilters()) {',
      ...
      '      // Only warn if we actually expected metadata but timed out',
      '      console.log(\'[IFC Filter] Setup finished: No IFC metadata found or timeout. Filters disabled.\')',
      '      app.off(\'update\', checkSetupHandler)',
      '      checkSetupHandler = null',
      '    }',
    ...
    // Create script file and hash it (same pattern as ScriptEditor.save())
    const scriptBlob = new Blob([ifcScriptCode], { type: 'text/javascript' })
    const scriptFile = new File([scriptBlob], 'ifc-filter.js', { type: 'text/javascript' })
    const scriptHash = await hashFile(scriptFile)
    const scriptFilename = `${scriptHash}.js`
    const scriptUrl = `asset://${scriptFilename}`
    
    // Cache script locally so this client can insta-load it
    ...
    this.select(app)
    // upload both the ifc file and script file (same pattern as addApp)
    await Promise.all([this.world.network.upload(file), this.world.network.upload(scriptFile)])
    // mark as uploaded so other clients can load it in
    app.onUploaded()
  }

...
import { ThatOpenIFCLoader } from './ThatOpenIFCLoader.js'

import { System } from './System'
import { createNode } from '../extras/createNode'
import { createVRMFactory } from '../extras/createVRMFactory'
import { glbToNodes } from '../extras/glbToNodes'
import { ifcToNodes } from '../extras/ifcToNodes'
...
export class ClientLoader extends System {
    ...
    const promise = this.loadFile(url).then(async file => {
      ...
      if (type === 'ifc') {
        try {
          ...
          const ifcModel = await this.thatOpenIFCLoader.loadAsync(url)
          ...
          console.error('[IFC] Error loading IFC file:', err)
          throw err
        }
      }
    })
  ...
  insert(type, url, file) {
    ...
    if (type === 'ifc') {
      promise = (async () => {
        try {
          console.log('[IFC] Loading via insert():', file.name)
          const ifcModel = await this.thatOpenIFCLoader.loadAsync(localUrl)
          const node = ifcToNodes(ifcModel, this.world)
